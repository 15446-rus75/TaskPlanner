...
public:
  ...
  storage::UserProgress getUserProgress() const override { return m_progress; }
  void updateUserProgress(const storage::UserProgress &progress) override { m_progress = progress; }
  void addXP(int amount, const QString &reason) override
  {
    Q_UNUSED(reason);
    m_progress.currentXP += amount;
  }
  void updateStreak(const QDate &currentDate) override { Q_UNUSED(currentDate); }
  int getCurrentLevel() const override { return m_progress.currentLevel; }
  int getTotalXP() const override { return m_progress.currentXP; }
  int getStreakDays() const override { return m_progress.streakDays; }
  QList< storage::Achievement > getAllAchievements() const override { return allAchievementsToReturn; }
  ...
  QList< QString > getUnlockedLocations() const override { return m_progress.unlockedLocations; }
  void unlockLocation(const QString &locationId) override { Q_UNUSED(locationId); }
  ...
  int getOnTimeCompletedCount() const override { return onTimeCompletedCount; }
  ...
  int getDeletedTasksCount() const override { return deletedTasksCount; }
  int getPerfectDaysCount() const override { return perfectDaysCount; }
  int getTotalLocationsCount() const override { return totalLocationsCount; }
  int getMaxTasksCompletedInOneDay() const override { return maxTasksCompletedInOneDay; }
  int getMaxHardTasksCompletedInOneDay() const override { return maxHardTasksCompletedInOneDay; }

  // Вспомогательные методы для настройки мока
  void setTasks(const QList< storage::Task > &tasks) { m_tasks = tasks; }
  void setProgress(const storage::UserProgress &progress) { m_progress = progress; }
  void setAllAchievements(const QList< storage::Achievement > &achievements)
  {
    allAchievementsToReturn = achievements;
  }

  // Счётчики вызовов (camelCase для совместимости с тестами)
  ...
  mutable int unlockAchievementCallCount = 0;
  ...
  mutable QString lastUnlockedAchievementId;

  // Настраиваемые возвращаемые значения
